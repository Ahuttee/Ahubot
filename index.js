...
if(msg.content.startsWith(prefix + "mcskin")) {
  if(args === 0) {
    return msg.reply("Usage: #mcskin <username>")
  }                 msg.channel.sendMessage(`**${args.join(" ")}**'s minecraft skin`)
                    msg.channel.sendFile( "https://visage.surgeplay.com/full/404/" + args.join(" ") + ".png")
}
...
if(msg.content.startsWith(prefix + "Sliced")) {
...
if(msg.content.startsWith(prefix + "realraft")) {
...
if(msg.content.startsWith(prefix + "im bored")) {
  msg.reply("go to markiplooliar's channel XDD")
}

  if (msg.content.startsWith(prefix + "ban")) {
    let modRole = msg.guild.roles.find("name", "VIP");
    if(!msg.member.roles.has(modRole)) {
        return msg.reply("you dont have `Premium#` role.. you need that to do that command!")
    }
    if(msg.mentions.users.size === 0) {
      return msg.reply("Hey! you forgot to mention a use first!");
    }
...
if(msg.content.startsWith(prefix + "restock")) {
  msg.channel.sendMessage("Restocking server..")
    msg.channel.sendMessage("Success. server has set to defaults")
}
...
if(msg.content.startsWith(prefix + "mail")) {
  ...
  client.users.get("195085272528977920").sendMessage(":e_mail: From " + "**" + msg.author.username + "**" + args.join(" \n\n"))
  }
...
.then(() => {
  ...
  .then((collected) => {
    ...
    .catch(() => {
      ...
      if(msg.content.startsWith(prefix + "channel.delete")) {
        msg.channel.delete(args.join(" "));
      }
...
//const prefix = "#";
  ...
  //if (curLevel > userData.level) {
    ...
    //message.reply(`You leveled up to **${curLevel}**! congratz`);
  //}
